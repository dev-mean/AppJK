function Interface(){
	...
	for(var i = 1; i<=36; i++){
		left = 58 + 36.75*j;
		value = val + 3*j;
		$("#bet_table").append('<div id="num_'+value+'" class="num num_normal" value="'+value+'" style="top: '+top+'px;left: '+left+'px; "></div>');
		j++;
		if(i%12 == 0){
			top = top + 48;
			j=0;
	...
	for (var i = two_to_one.length - 1; i >= 0; i--) {
		$("#bet_table").append('<div class="two_to_one" value="'+two_to_one[i]+'" style="top: '+top+'px; left: '+left+'px;"></div>');
		top = top + 48;
	};
